...
module: build
	...
	tar czf $(BIN_OUTPUT_PATH)/module.tar.gz $(BIN_OUTPUT_PATH)/cloudslam-wrapper meta.json

build: build-go

build-go:
	rm -f $(BIN_OUTPUT_PATH)/cloudslam-wrapper
	go build -tags no_cgo,osusergo,netgo -ldflags="-extldflags=-static $(COMMON_LDFLAGS)"  -o $(BIN_OUTPUT_PATH)/cloudslam-wrapper module.go
...
lint-go: tool-install
	...
	GOGC=50 go run github.com/golangci/golangci-lint/cmd/golangci-lint@v1.64.8 run -v --fix --config=./etc/golangci.yaml
